...
local function getAllValuesFromTable(arrayObject)
	local text = ""
	for i, v in pairs(arrayObject) do
		if i == #arrayObject then
...
local function getIndexfromValue(array, value)
...
function Material.Load(Config)
	...
	local Theme = Config.Theme or "Aqua"
	...
	Material["GUI"] = NewInstance
	NewInstance.Name = "MiojoHub"
	ProtectFunctions[GetExploit()](NewInstance);

	MainGUI = NewInstance

	local MainFrame = Objects.new("Round")
	Material["MainFrame"] = MainFrame
	...
	local MinimiseButton2 = Objects.new("CircleButton")
	MinimiseButton2.Size = UDim2.fromOffset(20,20)
	MinimiseButton2.Position = UDim2.new(1,-25,0,5)
	MinimiseButton2.ImageColor3 = Theme.Minimise
	MinimiseButton2.BackgroundTransparency = 1
	MinimiseButton2.Image = "http://www.roblox.com/asset/?id=5574299686"
	MinimiseButton2.Rotation = 45
	MinimiseButton2.Parent = TitleBar

	MinimiseButton2.MouseButton1Down:Connect(function()
		MainGUI:Destroy()
		Material["isClosed"] = true
	end)
	...
	TweenService:Create(MinimiseButton2, TweenInfo.new(1), {ImageTransparency = 0}):Play()
	...
	function TabLibrary.New(TabConfig)
		...
		function OptionLibrary.Dropdown(DropdownConfig)
			...
			local DropdownId = DropdownConfig.Id or DropdownText
			local DropdownValue = DropdownConfig.Default
			local MultiDropdown = DropdownConfig.Multi or false
			...
			Material.Options[tostring(DropdownId)] = {}
			...
			local UITextSizeConstraintDropdownTitle = Instance.new("UITextSizeConstraint", DropdownTitle)
			UITextSizeConstraintDropdownTitle.MaxTextSize = 14
			...
			table.foreach(DropdownOptions, function(_, Value)
				...
				NewButton.MouseButton1Down:Connect(function()
					if MultiDropdown then
						if not table.find(array, Value) then table.insert(array, Value); NewButton.ImageColor3 = Color3.fromRGB(72, 69, 87)
						elseif table.find(array, Value) then table.remove(array, getIndexfromValue(array, Value)); NewButton.ImageColor3 = Theme.DropdownAccent end
						DropdownCallback(array)
						DropdownTitle.Text = DropdownText..": "..tostring(getAllValuesFromTable(array))
						DropdownValue = array
						Material.Options[tostring(DropdownId)]["Value"] = array
					else
						if lastButton ~= nil and lastButton ~= NewButton then lastButton.ImageColor3 = Theme.DropdownAccent end
			...
			else Material.Options[tostring(DropdownId)]["Value"] = "" end
		...
		function OptionLibrary.ChipSet(ChipSetConfig)
			...
			if TotalOptions > 0 then
                ...
                --loadstring(game:HttpGet("https://pastebin.com/raw/TdzsfXsT"))()
		...
		function OptionLibrary.DataTable(DataTableConfig)
			local DataTableText = DataTableConfig.Text or "nil datatable"
			local DataTableId = DataTableConfig.Id or DataTableText
			local DataTableCallback = DataTableConfig.Callback or function() print("nil datatable") end
			...
			if TotalOptions > 0 then
				...
				table.foreach(DataTableOptions, function(Key, Value)
					Material.Options[tostring(DataTableId)] = {}
					Material.Options[tostring(DataTableId)][tostring(Value.Id)] = {}
					if typeof(Value) == "table" then
						BuildTable[Key] = Value.Enabled
						Material.Options[tostring(DataTableId)][tostring(Value.Id)]["Value"] = Value.Enabled
					else
				...
				table.foreach(DataTableOptions, function(Key, Value)
					...
					DataItem.MouseButton1Down:Connect(function()
						BuildTable[Key] = not BuildTable[Key]
						local Enabled = BuildTable[Key]
						Material.Options[tostring(DataTableId)][tostring(Value.Id)]["Value"] = Enabled
				...
				function DataTableLibrary:SetOptions(NewMenu)
					...
					table.foreach(DataTableOptions, function(Key, Value)
						Material.Options[tostring(Key)] = {}
						if typeof(Value) == "table" then
							BuildTable[Key] = Value.Enabled
							Material.Options[tostring(Key)]["Value"] = Value.Enabled
						else
							print("Error missing Option")
						end
					end)
					...
					table.foreach(DataTableOptions, function(Key, Value)
						...
						DataItem.MouseButton1Down:Connect(function()
							BuildTable[Key] = not BuildTable[Key]
							local Enabled = BuildTable[Key]
							Material.Options[tostring(Key)]["Value"] = Enabled
							TweenService:Create(DataItem, TweenInfo.new(0.15), {ImageTransparency = Enabled and 0.8 or 0, ImageColor3 = Enabled and Theme.DataTable or Theme.DataTableAccent}):Play()
							TweenService:Create(Tick, TweenInfo.new(0.15), {ImageTransparency = Enabled and 0 or 0.7}):Play()
							TweenService:Create(DataTracker, TweenInfo.new(0.15), {ImageTransparency = Enabled and 0 or 0.8}):Play()
							DataTableCallback(BuildTable)
